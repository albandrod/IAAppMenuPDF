...
def next_week_range_es(today=None) -> tuple[str, str]:
    """
    Semana siguiente (lunes a viernes) en formato dd/mm/yyyy
    """
    if today is None:
...
Reglas:
- No inventes platos: si faltan datos, pon lo que haya.
...
def weekly_menu_digest(weeklyTimer: func.TimerRequest) -> None:
    ...
    for prefix, display_name in menus_map.items():
        try:
            ...
            md_blocks.append(render_menu_block_md(display_name, menu))
